<?php
	...
	if($result == "a_login"){
		...
		if($conn){
			$p_uid = $_POST['p_uid'];
			$pk_id = $_POST['pk_id'];
			$http_url = 'http://agent.paybank.com/business/get_id.php';
		...
		$arr = json_encode($arr, JSON_UNESCAPED_UNICODE);
		echo $arr;
	}else if($result == "a_s_company_count"){
		if($conn){
				...
				$S_sql .= "(select sum(coin_amount) from s_company where pk_id != ''";
					if($company_number != ""){ $S_sql .= " AND company_number = '".$company_number."' "; }
					if($company_name != ""){ $S_sql .= " AND `company_name` = '".$company_name."' "; }
				$S_sql .= " ) AS `coin_amount`, ";


				$S_sql .= "(select sum(paybank_pax) from s_company where pk_id != '' ";
					if($company_number != ""){ $S_sql .= " AND company_number = '".$company_number."' "; }
					if($company_name != ""){ $S_sql .= " AND `company_name` = '".$company_name."' "; }
				$S_sql .= " ) AS `paybank_pax`, ";	

				$S_sql .= "(select sum(freezing_amount) from s_company where pk_id != '' ";
					if($company_number != ""){ $S_sql .= " AND company_number = '".$company_number."' "; }
					if($company_name != ""){ $S_sql .= " AND `company_name` = '".$company_name."' "; }
				$S_sql .= " ) AS `freezing_amount`, ";	

				$S_sql .= "(select sum(withdraw_amount) from s_company where pk_id != '' ";
					if($company_number != ""){ $S_sql .= " AND company_number = '".$company_number."' "; }
					if($company_name != ""){ $S_sql .= " AND `company_name` = '".$company_name."' "; }
				$S_sql .= " ) AS `withdraw_amount` ";	

				$S_sql .= " from s_company limit 1";
				...
				$arr = array(
					...
					"coin_amount" => ($row['coin_amount'] == null ? 0 : $row['coin_amount']),
					"paybank_pax" => ($row['paybank_pax'] == null ? 0 : $row['paybank_pax']),
					"freezing_amount" => ($row['freezing_amount'] == null ? 0 : $row['freezing_amount']),
					"withdraw_amount" => ($row['withdraw_amount'] == null ? 0 : $row['withdraw_amount']),
		...
		if($conn){
				...
				$S_sql = "select pk_id, company_name, company_number, coin_amount,paybank_pax,freezing_amount,withdraw_amount from s_company where pk_id !='' " ;
				if($company_name != ""){ $S_sql .= " AND company_name = ".$company_name." "; }
				if($company_number != ""){ $S_sql .= " AND company_number = ".$company_number." "; }
				$S_sql .= " order by pk_id desc limit ".$s_point.",".$list." ";
				...
				while($row = mysqli_fetch_array($result)){
					$db_list[] = array(
						...
						"company_number" => $row['company_number'],
						"coin_amount" => $row['coin_amount'],
						"paybank_pax" => $row['paybank_pax'],
						"freezing_amount" => $row['freezing_amount'],
		...
		$arr = json_encode($arr, JSON_UNESCAPED_UNICODE);
		echo $arr;
	}else if($result == "c_s_company_count"){
		...
		if($conn){
				...
				$S_sql .= "(select sum(coin_amount) from s_company where pk_id = ".$pk_id."";
				$S_sql .= " ) AS `coin_amount`, ";

				$S_sql .= "(select sum(freezing_amount) from s_company where pk_id = ".$pk_id."";
				$S_sql .= " ) AS `freezing_amount`, ";	

				$S_sql .= "(select sum(withdraw_amount) from s_company where pk_id = ".$pk_id."";
				$S_sql .= " ) AS `withdraw_amount` ";	
		...
		$arr = json_encode($arr, JSON_UNESCAPED_UNICODE);
		echo $arr;
	}else if($result == "c_update_money"){
		if($conn){
			
			$money = $_POST['money'];
			$pk_id = $_POST['pk_id'];
			
			$S_Withdraw = "select pk_id, status from s_withdraw where company_id = ".$pk_id." and status = 1 ";
			$W_result = mysqli_query($conn, $S_Withdraw);
			...
			if($W_row['pk_id']){
				...
				$S_sql = "select pk_id, paybank_id from s_company where coin_amount >= ".$money." and pk_id = ".$pk_id."";
				$result = mysqli_query($conn, $S_sql);
				$row = mysqli_fetch_array($result);
				...
				if($row['pk_id']){
					if($row['paybank_id'] == 0){
						...
						//s_company update
						$U_sql2 = "UPDATE s_company set coin_amount = coin_amount - ".$money.", freezing_amount = freezing_amount + ".$money." where pk_id = ".$pk_id." "; 
						$result2 = mysqli_query($conn, $U_sql2);
	
	
						$I_sql = "insert into s_withdraw(pk_id,company_id,withdraw_amount,status,memo,insert_time,update_time)values(null,".$pk_id.",".$money.",1,'','".$time."','".$time."')";
						$result3 = mysqli_query($conn, $I_sql);
	...
	}else if($result == "a_withdraw_count"){
		if($conn){
			...
			$S_sql = "select count(*) as cnt from s_withdraw W inner join s_company as C on C.pk_id = W.company_id where W.pk_id != '' ";
			if($company_name != ""){ $S_sql .= " AND C.company_name = ".$company_name." "; }
			if($type_status != 0){ $S_sql .= " AND W.status = ".$type_status." "; }
			if($start_date != ""){ $S_sql .= " AND W.insert_time > ".$start_date." "; }
			if($stop_date != ""){ $S_sql .= " AND W.insert_time < ".$stop_date." "; }
		...
		$arr = json_encode($arr, JSON_UNESCAPED_UNICODE);
		echo $arr;
	}else if($result == "a_withdraw_data"){
		if($conn){
			$company_name = $_POST['company_name'];
			$type_status = $_POST['type_status'];
            $start_date = $_POST['start_date'];
			...
			$S_sql = "select W.* ,C.company_name,C.paybank_id from s_withdraw W inner join s_company as C on C.pk_id = W.company_id where W.pk_id !='' ";
			if($company_name != ""){ $S_sql .= " AND C.company_name = ".$company_name." "; }
			if($type_status != 0){ $S_sql .= " AND W.status = ".$type_status." "; }
			if($start_date != ""){ $S_sql .= " AND W.insert_time > ".$start_date." "; }
			if($stop_date != ""){ $S_sql .= " AND W.insert_time < ".$stop_date." "; }

			$S_sql .= "order by W.pk_id desc limit ".$s_point.",".$list."";
			$result = mysqli_query($conn, $S_sql);
		...
		$arr = json_encode($arr, JSON_UNESCAPED_UNICODE);
		echo $arr;
	}else if($result == "c_withdraw_count"){
		if($conn){
			...
			$C_sql = "select count(*) as cnt from s_withdraw where company_id = ".$pk_id."";
			if($type_status != 0){$C_sql.= " AND status = ".$type_status." ";}
			if($start_date != ""){ $C_sql .= " AND insert_time > '".$insert_time."' "; }
			if($stop_date != ""){ $C_sql .= " AND insert_time < '".$update_time."' "; }
		...
		$arr = json_encode($arr, JSON_UNESCAPED_UNICODE);
		echo $arr;
	}else if($result == "c_withdraw_data"){
		...
		if($conn){
			...
			$C_sql = "select W.*, C.company_name from s_withdraw W inner join s_company as C on C.pk_id = W.company_id where W.company_id = ".$pk_id."";
			if($type_status != 0){$C_sql.= " AND W.status = ".$type_status." ";}
			if($start_date != ""){ $C_sql .= " AND W.insert_time > '".$insert_time."' "; }
			if($stop_date != ""){ $C_sql .= " AND W.insert_time < '".$update_time."' "; }
			$C_sql .= " order by W.pk_id desc limit ".$s_point.",".$list." ";
			$result = mysqli_query($conn, $C_sql);
		...
		$arr = json_encode($arr, JSON_UNESCAPED_UNICODE);
		echo $arr;
	}else if($result == "a_withdraw_confirm"){
		if($conn){
			
			$pk_id = $_POST['pk_id'];
			$time = time();
			$S_sql = "select withdraw_amount,company_id from s_withdraw where pk_id = ".$pk_id."";
			$result = mysqli_query($conn, $S_sql);
			$row = mysqli_fetch_array($result);

			if($row['company_id']){
				//company id 있으면 s_withdraw 출금상태 및 업데이트 시간 update
				$U_sql = "update s_withdraw set status = 2, update_time = '".$time."' where pk_id = ".$pk_id."";
				mysqli_query($conn, $U_sql);

				//s_company update

				$U_sql2 = "update s_company set freezing_amount = freezing_amount - ".$row['withdraw_amount'].", withdraw_amount = withdraw_amount + ".$row['withdraw_amount']." where pk_id = ".$row['company_id']."";
				mysqli_query($conn, $U_sql2);
		...
		$arr = json_encode($arr, JSON_UNESCAPED_UNICODE);
		echo $arr;
	}else if($result == "a_withdraw_confirm_cancer"){
		if($conn){
			...
			if($row['company_id']){
				//company id 있으면 s_withdraw 출금상태 및 업데이트 시간 update
				$U_sql = "update s_withdraw set status = 3, update_time = '".$time."' where pk_id = ".$pk_id."";
				mysqli_query($conn, $U_sql);

				//s_company update

				$U_sql2 = "update s_company set freezing_amount = freezing_amount - ".$row['withdraw_amount'].", coin_amount = coin_amount + ".$row['withdraw_amount']." where pk_id = ".$row['company_id']."";
				mysqli_query($conn, $U_sql2);
		...
		if($conn){
			...
			$U_sql = "update admin set token = '".$j_token."', random_key = '".$j_randomKey."', account ='".$linked_account."'";
	...
	}else if($result == "a_get_phone_number"){
		if($conn){
			$pk_id = $_POST['pk_id'];
			$paybank_id = $_POST['paybank_id'];

			$http_url = 'http://agent.paybank.com/business/get_id.php';

			$row_p = post($http_url, array('result'=>'u_queren_button', 'data'=>$paybank_id));
			$row_p = json_decode($row_p, JSON_UNESCAPED_UNICODE);
		...
		$arr = json_encode($arr, JSON_UNESCAPED_UNICODE);
		echo $arr;
	}else if ($result == "a_check_withdraw"){
		if($conn){

			$S_sql = "select count(*) as cnt from s_withdraw where status = 1";
			$result = mysqli_query($conn, $S_sql);
	...
	}else if($result == "a_check_history"){
		if($conn){
			
			$c_pk_id = $_POST['pk_id'];
			$S_sql = "select count(*) as cnt from s_order_history H inner join s_product as P on P.pk_id = H.product_id inner join s_company as C on C.pk_id = P.company_id where H.type = 1 and C.pk_id = ".$c_pk_id."";
			$result = mysqli_query($conn, $S_sql);
			$row = mysqli_fetch_array($result);
	...
	}else if($result == "a_return_product"){
		if($conn){
			$company_name = $_POST['company_name'];
            $type_status = $_POST['type_status'];
            $start_date = $_POST['start_date'];
			...
			$S_sql = "select count(*) as cnt from s_return_request R inner join s_company as C on C.pk_id = R.company inner join s_member as M on M.pk_id = R.member_id where R.pk_id != ''";
			if($company_name != ""){ $S_sql .= " AND C.company_name = ".$company_name." "; }
			if($email != ""){ $S_sql .= " AND M.email = '".$email."' "; }
			if($type_status != 99){ $S_sql .= " AND R.status = ".$type_status." "; }
			if($order_num != ""){ $S_sql .= " AND R.order_num = '".$order_num."' "; }
			if($deliver_num != ""){ $S_sql .= " AND R.invoice_number = '".$deliver_num."' "; }
			if($start_date != ""){ $S_sql .= " AND R.insert_time > '".$start_date."' "; }
			if($stop_date != ""){ $S_sql .= " AND R.insert_time < '".$stop_date."' "; }
		...
		echo $arr;
			...
			$C_sql = "select W.*, C.company_name,M.email,M.p_account,M.p_id from s_return_request W inner join s_company as C on C.pk_id = W.company inner join s_member as M on M.pk_id = W.member_id ";
			if($type_status != 99){$C_sql.= " AND W.status = ".$type_status." ";}
			if($email != ""){$C_sql.= " AND M.email = '".$email."' ";}
			if($order_num != ""){ $C_sql .= " AND W.order_num = '".$order_num."' "; }
			if($deliver_num != ""){ $C_sql .= " AND W.invoice_number = '".$deliver_num."' "; }
			if($start_date != ""){ $C_sql .= " AND W.insert_time > '".$insert_time."' "; }
			if($stop_date != ""){ $C_sql .= " AND W.insert_time < '".$update_time."' "; }
			$C_sql .= " order by W.pk_id desc limit ".$s_point.",".$list." ";
			$result = mysqli_query($conn, $C_sql);
			...
			while($row = mysqli_fetch_array($result)){
				$db_list[] = array(
					...
					"product_price" => $row['product_price'],
					"quantity" => $row['quantity'],
					"return_delivery" => $row['return_delivery'],
		...
		$arr = json_encode($arr, JSON_UNESCAPED_UNICODE);
		echo $arr;
	}else if($result == "c_return_product"){
		if($conn){
			$pk_id = $_POST['pk_id'];
			$email = $_POST['email'];
			$company_name = $_POST['company_name'];
			...
			$S_sql = "select count(*) as cnt from s_return_request R inner join s_company as C on C.pk_id = R.company inner join s_member as M on M.pk_id = R.member_id where R.company = ".$pk_id." ";
			if($type_status != 99){ $S_sql .= " AND R.status = ".$type_status." "; }
			if($email != ""){ $S_sql .= " AND M.email = '".$email."' "; }
			if($order_num != ""){ $S_sql .= " AND R.order_num = '".$order_num."' "; }
			if($deliver_num != ""){ $S_sql .= " AND R.invoice_number = '".$deliver_num."' "; }
		...
		if($conn){
			...
			$C_sql = "select W.*, C.company_name,M.email from s_return_request W inner join s_company as C on C.pk_id = W.company inner join s_member as M on M.pk_id = W.member_id where W.company = ".$pk_id."";
			if($type_status != 99){$C_sql.= " AND W.status = ".$type_status." ";}
			if($email != ""){ $C_sql .= " AND M.email = '".$email."' "; }
			if($order_num != ""){ $C_sql .= " AND W.order_num = '".$order_num."' "; }
			if($deliver_num != ""){ $C_sql .= " AND W.invoice_number = '".$deliver_num."' "; }
		...
		$arr = json_encode($arr, JSON_UNESCAPED_UNICODE);
		echo $arr;
	}else if($result == "c_update_return"){
		if($conn){

			$return_pk_id = $_POST['return_pk_id'];
			$U_sql = "update s_return_request set status = 2 where pk_id = ".$return_pk_id."";
			$result = mysqli_query($conn, $U_sql);
		...
		$arr = json_encode($arr, JSON_UNESCAPED_UNICODE);
		echo $arr;
	}else if($result == "a_refund"){
		if($conn){
			$email  = $_POST['email'];
			$pk_id = $_POST['pk_id'];


			//s_return_request update 

			$U_sql = "update s_return_request set status = 1, money_back = 1 where pk_id = ".$pk_id." ";
			$result = mysqli_query($conn, $U_sql);
		...
		$arr = json_encode($arr, JSON_UNESCAPED_UNICODE);
		echo $arr;
	}else if($result == "a_check_refund"){
		if($conn){

			$S_sql = "select count(*) as cnt from s_return_request where status = 2";
			$result = mysqli_query($conn, $S_sql);
