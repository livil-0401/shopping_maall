<?php
	...
	if($result == "a_login"){
			...
			if($img_file_1 && $img_file_name_1 != "" && $img_file_2 && $img_file_name_2 != ""){
				...
				if(!in_array($ext_i, $allowed) || !in_array($ext_2, $allowed)){ 
					...
					if($_FILES['i_company_img_1']['size'] > $max_file_size || $_FILES['i_company_img_2']['size'] > $max_file_size){
						...
						$password = md5("1234");
						$password = md5($password);
						$sql_insert = " INSERT INTO `s_company` (`pk_id`,`company_name`,`company_address`,`company_contact`,`company_number`,`company_type`,`password`,`status`,`insert_time`,`update_time`,`coin_amount`,`company_img_1`,`company_img_2`,`return_address`) VALUES ";
						$sql_insert .= "(NULL,'".$i_company_name."','".$i_company_address."','".$i_company_contact."','".$i_company_number."',".$i_company_type.",'".$password."',1,'".$time."','".$time."',0,'".$img_name_1."','".$img_name_2."','".$i_return_address."')";
						mysqli_query($conn, $sql_insert);
					}
				}
		...
		if($conn){
			$sql_select = " SELECT `pk_id`,`status`,`company_name`,`company_address`,`company_contact`,`otp_ck`,`paybank_id` FROM `s_company` WHERE `company_number` = '".$_COOKIE["c_mall_company_number"]."' AND `password` = '".$_COOKIE["c_mall_company_password"]."' ";
			$result = mysqli_query($conn, $sql_select);
			$row = mysqli_fetch_array($result);
		...
		if($conn){
            ...
            //select product_data 
             ...
             $S_sql = "select * from s_product order by pk_id desc limit ".$s_point.", ".$list."";
        ...
        echo $arr;
		  ...
		  //현재 시간
            ...
            //이미지 경로 설정 
			print "<script language=javascript> alert('등록 성공'); location.replace('./admin/product.html');</script>";
		...
		if($conn){
			...
			$U_sql = "update s_company set company_name = '".$username."', company_contact = '".$mobile."' where pk_id = ".$pk_id."";
		...
		if($conn){
			...
			$S_sql .= " where pk_id desc limit ".$s_point.",".$list."";
